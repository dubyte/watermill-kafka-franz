...
| Test | What it verifies | Requires Toxiproxy |
|---|---|---|
| `TestSubscriber_StopAllowsInflightToAck` | In-flight msg can be Acked after Stop; new Subscribe rejected | No |
| `TestSubscriber_CloseSubscribeRace` | No goroutine outlives Close(); run with -race | No |
| `TestSubscriber_StopThenClose_NoDeadlock` | Stop then Close returns within 10 s | No |
...
| Test | What it verifies | Requires Toxiproxy |
|---|---|---|
| `TestSubscriber_PoisonPill_DoesNotBlockSubsequentMessages` | Valid msgs after poison pill are delivered | No |
| `TestSubscriber_PoisonPill_PartitionDoesNotStallAfterRebalance` | Poison pill skipped after rebalance | No |
| `TestSubscriber_PoisonPill_ErrorIsLogged` | Error entry contains topic/partition/offset | No |
...
| `TestSubscriber_Network_CommitTimeout_UnderDisableAutoCommit` | Commit failure is propagated, not swallowed |
...
| `TestSubscriber_Network_IsClientClosedSpin_Fix` | Close() returns within 10 s when client self-closes |
...
| `TestSubscriber_SliceGrowth_OnContextCancelledResubscribe` | subClients does not grow on reconnect cycles | No |
